	...
	**/
	...
	@:noCompletion private function __getNextFrame(deltaTime:Float):Int
	{
		...
		if (frameRate != null)
		{
			...
			nextFrame = __currentFrame + inline Math.floor(__timeElapsed / __frameTime);
			if (nextFrame < 1) nextFrame = 1;
			if (nextFrame > __totalFrames) nextFrame = inline Math.floor((nextFrame - 1) % __totalFrames) + 1;
