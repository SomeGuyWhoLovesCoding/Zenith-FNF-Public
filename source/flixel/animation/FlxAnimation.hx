...
class FlxAnimation extends FlxBaseAnimation
{
	...
	public function play(Force:Bool = false, Reversed:Bool = false, Frame:Int = 0):Void
	{
		...
		@:privateAccess parent._sprite.draw(); // Make sure to call draw immediately before this function
		...
		if (finished)
			inline parent.fireFinishCallback(name);
	}
	...
	inline function set_curFrame(frame:Int):Int
	{
		var maxFrameIndex:Int = numFrames - 1;
