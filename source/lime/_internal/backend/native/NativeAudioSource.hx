...
class NativeAudioSource
{
	...
	public function init():Void
	{
		...
		if (parent.buffer.__srcVorbisFile != null)
		{
			...
			var vorbisFile:VorbisFile = parent.buffer.__srcVorbisFile;
			dataLength = Int64.toInt(vorbisFile.pcmTotal()) * parent.buffer.channels * (parent.buffer.bitsPerSample * 0.125);
	...
	public function getCurrentTime():Float
	{
		...
		else if (handle != null)
		{
			if (stream)
			{
				var time:Float = ((bufferTimeBlocks[0] + AL.getSourcef(handle, AL.SEC_OFFSET)) * 1000.0) - parent.offset;
				if (time < 0) return 0;
				return time;
			}
			else
	...
	public function setCurrentTime(value:Float):Float
	{
		if (handle != null)
		{
			...
			else if (parent.buffer != null)
			{
				...
				var totalOffset:Float = dataLength * ratio;
	...
	inline public function getLength():Float
		return length > 0 ? length : samples / parent.buffer.sampleRate * 1000.0 - parent.offset;
