...
class NativeApplication
{
	...
	private function handleKeyEvent():Void
	{
		...
		if (window != null)
		{
			...
			var keyCode:Float = keyEventInfo.keyCode;
			var modifier:KeyModifier = keyEventInfo.modifier;

			switch (type)
			{
				case KEY_DOWN:
					window.onKeyDown.dispatch(Std.int(keyCode), modifier);
				...
				case KEY_UP:
					window.onKeyUp.dispatch(Std.int(keyCode), modifier);

					switch (keyCode)
					{
						#if android
						case KeyCode.APP_CONTROL_BACK:
							if (modifier != KeyModifier.NONE || window.onKeyUp.canceled)
								return;
