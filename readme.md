...
Lime: ``haxelib git lime https://github.com/openfl/lime.git`` (If lime isn't set to git, delete the previous one and do ``haxelib set lime git``) *Works if not on neko, but useful for hasklink* (or just ``haxelib install lime``)
